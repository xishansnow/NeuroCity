...
class TestGridNeRFLoss(unittest.TestCase):
    """Test GridNeRFLoss class."""
    ...
    def test_color_loss(self):
        """Test color loss computation."""
        pred_rgb = torch.rand(20, 3)
        target_rgb = torch.rand(20, 3)
        
        loss = self.loss_fn.color_loss(pred_rgb, target_rgb)
    ...
    def test_full_loss(self):
        ...
        # Mock model for regularization
        mock_model = self.loss_fn
        mock_model.hierarchical_grid = type('MockGrid', (), {
            'grids': [torch.rand(8, 16, 16, 16) for _ in range(2)]
        })()
        
        loss_dict = self.loss_fn(outputs, target_rgb, batch)
        
        self.assertIn('total_loss', loss_dict)
        self.assertIn('color_loss', loss_dict)
...
class TestUtilities(unittest.TestCase):
    ...
    def test_sample_along_rays(self):
        ...
        points, z_vals = sample_along_rays(
            rays_o, rays_d, near=1.0, far=10.0, n_samples=32
        )
...
class TestIntegration(unittest.TestCase):
    ...
    def test_gradient_flow(self):
        ...
        outputs = model(rays_o, rays_d)
        loss_dict = loss_fn(outputs, target_rgb, {})
