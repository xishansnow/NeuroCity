...
# Local imports
from src.nerfs.mega_nerf_plus.core import (
    MegaNeRFPlus,
    MegaNeRFPlusConfig,
    HierarchicalSpatialEncoder,
)
from src.nerfs.mega_nerf_plus.spatial_partitioner import (
    AdaptiveOctreePartitioner,
    PhotogrammetricPartitioner,
    PartitionConfig,
)
from src.nerfs.mega_nerf_plus.multires_renderer import (
    MultiResolutionRenderer,
    AdaptiveLODRenderer,
)
from src.nerfs.mega_nerf_plus.memory_manager import (
    MemoryManager,
    CacheManager,
    MemoryOptimizer,
)
from src.nerfs.mega_nerf_plus.dataset import (
...
class TestMegaNeRFPlusCore:
    ...
    def test_different_lod_levels(self):
        ...
        rays_d = torch.nn.functional.normalize(torch.randn(8, 3, device=self.device), dim=-1)
...
class TestSpatialPartitioning:
    ...
    def setup_method(self):
        """Setup test environment"""
        self.config = PartitionConfig(max_partition_size=64, min_partition_size=16, max_depth=3)
    ...
    def test_adaptive_octree(self):
        ...
        # Create partitioner with scene bounds
        partitioner = AdaptiveOctreePartitioner(
            scene_bounds=tuple(scene_bounds.flatten().tolist()), config=self.config
        )
        ...
        assert all("centroid" in p for p in partitions)
...
class TestMultiResolutionRenderer:
    ...
    def setup_method(self):
        """Setup test environment"""
        self.config = MegaNeRFPlusConfig(num_lods=3, num_samples=16, num_importance=32)
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
...
class TestPerformance:
    ...
    def test_rendering_speed(self):
        ...
        rays_d = torch.nn.functional.normalize(torch.randn(100, 3, device=self.device), dim=-1)
