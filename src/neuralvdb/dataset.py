...
class NeuralVDBDataset(Dataset):
    ...
    def get_statistics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'occupancy_mean': float(np.mean(occupancies_np)),
            'occupancy_std': float(np.std(occupancies_np)),
            'occupancy_min': float(np.min(occupancies_np)),
            'occupancy_max': float(np.max(occupancies_np)),
            'occupied_ratio': float(np.mean(occupancies_np > 0.5)),
            'bbox_min': points_np.min(axis=0).tolist(),
            'bbox_max': points_np.max(axis=0).tolist(),
            'bbox_size': (points_np.max(axis=0) - points_np.min(axis=0)).tolist()
        }
...
class VoxelDataset(Dataset):
    ...
    def get_statistics(self) -> Dict[str, float]:
        ...
        stats = {
            ...
            'target_min': float(np.min(targets_np)),
            'target_max': float(np.max(targets_np)),
            'bbox_min': coords_np.min(axis=0).tolist(),
            'bbox_max': coords_np.max(axis=0).tolist(),
            'bbox_size': (coords_np.max(axis=0) - coords_np.min(axis=0)).tolist()
        }
...
class TileDataset(Dataset):
    ...
    def __init__(self, 
                 tiles_dir: str,
                 tile_indices: Optional[List[Tuple[int, int]]] = None,
                ...
                if filename.endswith('.npy') and filename.startswith('tile_'):
                    ...
                    if len(parts) >= 3:
                        try:
                            ...
                            json_path = os.path.join(self.tiles_dir, filename.replace('.npy', '.json'))
    ...
    def _load_single_tile(self, tile_file: Dict) -> Dict:
        ...
        return {
            'tile_index': tile_file['tile_index'],
            'coords': coords,
            'values': values,
    ...
    def get_tile_statistics(self) -> Dict[str, Any]:
        ...
        for i in range(len(self)):
            ...
            tile_info.append({
                ...
                'occupancy_ratio': occupied_count / voxel_count,
        ...
        return {
            ...
            'global_occupancy_ratio': total_occupied / total_voxels,
            'tile_info': tile_info
        }
...
class DataTransforms:
    ...
    @staticmethod
    def normalize_coords(coords: torch.Tensor, 
    ...
    @staticmethod
    def random_rotation(coords: torch.Tensor, 
                       max_angle: float = 0.1) -> torch.Tensor:
        """随机旋转（仅绕Z轴）"""
    ...
    @staticmethod
    def random_scale(coords: torch.Tensor, 
                    scale_range: Tuple[float, float] = (0.9, 1.1)) -> torch.Tensor:
        """随机缩放"""
