...
class GridNeRFDataset(Dataset):
    ...
    def __init__(self, data_root: str | Path, split: str = "train", **kwargs):
        """Initialize dataset."""
        super().__init__()
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor | int | None]:
        """Get a batch of data."""
        if self.split == "train":
            ...
            return {"ray_origins": rays_o, "ray_directions": rays_d, "rgb": rgb, "image_idx": idx}
        else:
            ...
            return {
                ...
                "rgb": self.images[idx],
                "image_idx": idx,
                "metadata": self.metadata[idx] if hasattr(self, "metadata") else None,
            }
    ...
    def generate_rays(
        self, camera: dict[str, torch.Tensor], image_shape: tuple[int, int]
...
class KITTI360GridDataset(GridNeRFDataset):
    ...
    def load_data(self):
        ...
        for i, image_file in enumerate(image_files):
            ...
            sample = {"image_path": str(image_file), "camera_id": f"cam_{self.camera_id}"}
...
class WaymoGridDataset(GridNeRFDataset):
    ...
    def __init__(
        self, data_root: str, segment_name: str, camera_name: str = "FRONT", *args, **kwargs
    ):
    ...
    def load_data(self):
        ...
        for image_file in image_files:
            ...
            if timestamp in poses:
                ...
                sample = {"image_path": str(image_file), "camera_id": f"{self.camera_name}"}
