...
def hierarchical_sampling(
    ...
    bins_g = torch.gather(t_vals_coarse.unsqueeze(-2), dim=-1, index=indices.unsqueeze(-1))
    ...
    fine_points = ray_origins.unsqueeze(-2) + t_vals_combined.unsqueeze(
        -1
...
def voxel_traversal_3d(
    ...
    for _ in range(max_steps):
        ...
        for dim in range(3):
            if step[..., dim] != 0:
                t_dim = (
                    (voxel[..., dim] + step[..., dim]) * cell_size[dim]
                    + grid_min[dim]
                    - ray_origins[..., dim]
                ) / ray_directions[..., dim]
                mask = t_dim < t_next
                t_next = torch.where(mask, t_dim, t_next)
                next_voxel = torch.where(
                    mask.unsqueeze(-1), voxel + step * torch.eye(3)[dim], next_voxel
                )
...
def importance_sampling_from_grid(
    ...
    voxel_indices, t_starts, t_ends = voxel_traversal_3d(
        ray_origins, ray_directions, grid_bounds, grid_resolution
    )
...
def get_ray_directions(
    height: int,
    width: int,
    focal_length: Union[float, torch.Tensor],
    ...
    Args:
        height: Image height
        width: Image width
        focal_length: Focal length of the camera (can be [fx, fy] or single value)
        center: Optional camera center (principal point). If None, use image center
        device: Device to use
    ...
    # Convert focal length to tensor if needed
    ...
    # Set camera center to image center if not provided
    if center is None:
        center = torch.tensor(
            [[width / 2, height / 2]],
            device=device,
    ...
    x = (i - center[0, 0]) / focal_length[0]
    y = -(j - center[0, 1]) / focal_length[1]
    z = -torch.ones_like(x)

    # Stack and normalize directions
