...
class GridNeRFTrainer:
    ...
    def setup_model(self) -> None:
        ...
        self.loss_fn = GridNeRFLoss(self.config).to(self.device)
    ...
    def train_step(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        loss_dict_cpu = {k: v.item() if torch.is_tensor(v) else v 
    ...
    def train(
        ...
        for epoch in range(self.current_epoch, self.config.num_epochs):
            ...
            # Render test images
            if (test_dataset and self.rank == 0 and 
                (epoch + 1) % self.config.render_every_n_epochs == 0):
