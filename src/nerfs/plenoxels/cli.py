"""
Command Line Interface for Plenoxels

Provides command-line tools for training and rendering with Plenoxels.
"""
...
def train_cli():
    """CLI for training Plenoxels models."""
    parser = argparse.ArgumentParser(description="Train Plenoxels model")
    ...
    # Model arguments
    parser.add_argument("--grid-resolution", type=int, default=128, help="Grid resolution")
    ...
    try:
        from ..plenoxels import (
            ...
            Plenoxels,
            PlenoxelsTrainer,
            PlenoxelsTrainerConfig,
        )
        from ..plenoxels.dataset import PlenoxelsDataset, PlenoxelsDatasetConfig
    except ImportError as e:
        logger.error(f"Failed to import plenoxels: {e}")
        sys.exit(1)
    ...
    # Create model
    model = Plenoxels(model_config)
    logger.info(f"Model created with {sum(p.numel() for p in model.parameters()):,} parameters")

    # Create trainer configuration
    trainer_config = PlenoxelsTrainerConfig(
    ...
    # Create trainer
    trainer = PlenoxelsTrainer(model, trainer_config)
    trainer = trainer.to(device)

    # Create dataset
    try:
        dataset_config = PlenoxelsDatasetConfig(
            data_root=args.data_dir,
            split="train",
        )

        train_dataset = PlenoxelsDataset(dataset_config)
        val_dataset = PlenoxelsDataset(PlenoxelsDatasetConfig(data_root=args.data_dir, split="val"))
...
def render_cli():
    """CLI for rendering with trained Plenoxels models."""
    parser = argparse.ArgumentParser(description="Render with Plenoxels model")
    ...
    # Create renderer
    renderer_config = PlenoxelsRendererConfig(
    ...
    renderer = PlenoxelsRenderer(model, renderer_config)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "train":
            ...
            print("Usage: python -m plenoxels.cli [train|render] [args...]")
            sys.exit(1)
    else:
        print("Usage: python -m plenoxels.cli [train|render] [args...]")
        sys.exit(1)
