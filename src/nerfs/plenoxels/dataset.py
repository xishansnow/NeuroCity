...
def load_colmap_data(
    ...
    return {"images": imgs, "poses": poses, "hwf": hwf, "K": K, "near": 0.1, "far": 100.0}
...
class PlenoxelDataset(Dataset):
    ...
    def __init__(self, config: PlenoxelDatasetConfig, split: str = "train") -> None:
        ...
        if config.dataset_type == "blender":
            self.data = load_blender_data(config.data_dir, half_res=(config.downsample_factor > 1,))
        elif config.dataset_type == "colmap":
            self.data = load_colmap_data(config.data_dir, factor=config.downsample_factor)
        else:
    ...
    def get_test_poses(self, n_poses: int = 40) -> np.ndarray:
        ...
        # Create spiral path
        render_poses = []
        for theta in np.linspace(0.0, 2.0 * np.pi, n_poses, endpoint=False):
