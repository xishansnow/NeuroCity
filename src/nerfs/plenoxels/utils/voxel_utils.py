...
def create_voxel_grid(
    ...
    voxel_size = torch.tensor(
        [(x_max - x_min) / W, (y_max - y_min) / H, (z_max - z_min) / D], device=device
    )
...
def compute_voxel_bounds(
    ...
    normalized_coords = torch.stack([(coords[:, 0] - x_min,)], dim=-1)
...
def voxel_to_world_coords(
    ...
    world_coords = torch.stack([normalized_coords[:, 0] * (x_max - x_min,)], dim=-1)
...
def world_to_voxel_coords(
    ...
    # Normalize to [0, 1]
    normalized_coords = torch.stack([(world_coords[:, 0] - x_min,)], dim=-1)

    # Scale to voxel indices
    voxel_indices = torch.stack([normalized_coords[:, 0] * (W - 1,)], dim=-1)
