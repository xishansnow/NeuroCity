...
class MipNeRFDataset(data.Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            near_far: tuple of (near, far) for clipping rays
        """
    ...
    def __getitem__(self, idx):
        return {"rays": self.rays[idx], "rgbs": self.rgbs[idx]}
    ...
    def get_rays(self, pose: torch.Tensor, H: int, W: int, focal: float) -> dict[str, torch.Tensor]:
        ...
        return {"rays_o": rays_o, "rays_d": rays_d, "radii": radii}
...
class LLFFMipNeRFDataset(MipNeRFDataset):
    ...
    def _load_data(self):
        ...
        imgfiles = sorted(
            list(imgdir.glob("*.JPG")) + list(imgdir.glob("*.jpg")) + list(imgdir.glob("*.png"))
        )

        if len(imgfiles) != poses.shape[0]:
            raise ValueError(
                f"Number of images ({len(imgfiles)}) doesn't match number of poses ({poses.shape[0]})"
            )
...
class MipNeRFRayDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {"rays_o": rays_o, "rays_d": rays_d, "radii": radii, "rgbs": batch_rgbs}
...
def create_mip_nerf_dataloader(
    dataset: MipNeRFDataset | MipNeRFRayDataset,
    batch_size: int = 4096,
    shuffle: bool = True,
    num_workers: int = 4,
