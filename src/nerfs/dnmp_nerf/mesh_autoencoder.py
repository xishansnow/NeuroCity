...
class MeshDecoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_dims: Tuple[int, ...] = (256, 512, 1024, 512),
        ...
        if self.use_residual and num_vertices is not None:
            self.register_buffer('base_vertices', 
                               self._generate_base_mesh(num_vertices, output_dim))
...
class MeshAutoEncoder(nn.Module):
    ...
    def compute_loss(self, vertices: torch.Tensor, 
    ...
    def interpolate(self, vertices1: torch.Tensor, vertices2: torch.Tensor,
...
def create_mesh_autoencoder(config_dict: dict) -> MeshAutoEncoder:
    ...
    """
    return MeshAutoEncoder(
        latent_dim=config_dict.get('latent_dim', 128),
        num_vertices=config_dict.get('num_vertices', None),
        hidden_dims=tuple(config_dict.get('hidden_dims', [256, 512, 256])),
        use_variational=config_dict.get('use_variational', False)
    )


def pretrain_mesh_autoencoder(autoencoder: MeshAutoEncoder,
                             mesh_dataset: torch.utils.data.DataLoader,
                             num_epochs: int = 100,
                             learning_rate: float = 1e-3,
                             device: torch.device = None) -> MeshAutoEncoder:
    """
