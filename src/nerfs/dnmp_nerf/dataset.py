...
class DNMPDataset(data.Dataset):
    ...
    def generate_rays(self, camera: dict[str, torch.Tensor], image_shape: tuple[int, int]):
...
class UrbanSceneDataset(DNMPDataset):
    ...
    def load_data(self):
        ...
        if metadata_path.exists():
            ...
            self.cameras = {cam["id"]: self._parse_camera(cam) for cam in metadata["cameras"]}
...
class KITTI360Dataset(DNMPDataset):
    ...
    def __init__(
        self, data_root: str, sequence: str = "2013_05_28_drive_0000_sync", *args, **kwargs
    ):
...
def create_dataloader(
    dataset: DNMPDataset, batch_size: int = 1, shuffle: bool = True, num_workers: int = 4
):
