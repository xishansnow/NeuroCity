...
class NerfactoTrainer:
    ...
    def _create_scheduler(self) -> optim.lr_scheduler.LRScheduler:
        ...
        if self.config.lr_scheduler == "exponential":
            # Exponential decay
            gamma = (self.config.lr_final / self.config.learning_rate) ** (
                1.0 / self.config.max_epochs
            )
    ...
    def validate(self) -> Dict[str, float]:
        ...
        return {"val_loss": np.mean(val_losses), "val_psnr": np.mean(val_psnrs)}
    ...
    def save_checkpoint(self, is_best: bool = False) -> None:
        ...
        checkpoint_path = os.path.join(
            self.output_dir, "checkpoints", f"checkpoint_epoch_{self.epoch}.pth"
        )
        ...
        for epoch in range(self.config.max_epochs):
            ...
            for batch_idx, batch in enumerate(progress_bar):
                ...
                if self.step % self.config.log_every_n_steps == 0:
                    ...
                    # Update progress bar
                    progress_bar.set_postfix({"loss": f"{losses['total_loss']:.6f}"})
            ...
            if epoch % self.config.eval_every_n_epochs == 0:
                ...
                if val_metrics:
                    ...
                    print(
                        f"Validation - Loss: {val_metrics.get('val_loss', 0):.6f}, "
            ...
            print(
                f"Epoch {epoch} completed - Avg Loss: {avg_loss:.6f}, "
...
def create_nerfacto_trainer(
    ...
    **kwargs: Any,
    ...
    dataset_config = NerfactoDatasetConfig(data_dir=data_dir)
