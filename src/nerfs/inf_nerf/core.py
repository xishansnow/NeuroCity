...
class OctreeNode:
    ...
    def contains_point(self, point) -> bool:
        ...
        # 确保 point 是 numpy 数组，与 bounds_min/bounds_max 类型一致
    ...
    def find_containing_child(self, point: np.ndarray) -> "OctreeNode | None":
...
class SphericalHarmonicsEncoder(nn.Module):
    ...
    def forward(self, directions: torch.Tensor) -> torch.Tensor:
        ...
        # Degree 2
        if self.degree >= 2:
            features.extend([x * y, x * z, y * z, x * x - y * y, 3 * z * z - 1])
...
class InfNeRF(nn.Module):
    ...
    def build_octree(self, sparse_points: np.ndarray, max_depth: int | None = None):
        """
    ...
    def _build_octree_recursive(self, node: OctreeNode, sparse_points: np.ndarray, max_depth: int):
        ...
        if node.level >= max_depth or len(points_in_node) < self.config.sparse_point_threshold:
            return
    ...
    def forward(
        ...
        **kwargs,
        ...
        radii = self.renderer.calculate_sample_radius(z_vals, focal_length, pixel_width)
        ...
        for i in range(batch_size):
            ...
            for j in range(self.config.num_samples):
                ...
                with torch.no_grad() if not self.training else torch.enable_grad():
                    result = node.nerf(
                        sample_pos.unsqueeze(0), rays_d[i : i + 1]  # [1, 3]  # [1, 3]
                    )
