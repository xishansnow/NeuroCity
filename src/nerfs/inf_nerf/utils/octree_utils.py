...
class OctreeBuilder:
    ...
    def build_from_sparse_points(
        ...
        # Create root node
        root_node = OctreeNode(center=scene_center, size=scene_size, level=0, config=self.config)
    ...
    def _build_recursive(self, node: OctreeNode, sparse_points: np.ndarray, current_depth: int):
        ...
        if (
            ...
            or len(points_in_node) < self.config.sparse_point_threshold
        ):
            return
...
def visualize_octree(
    ...
    ax.set_zlabel("Z")
    ax.set_title(
        f"InfNeRF Octree Visualization\n"
