...
class DistributedRenderer:
    ...
    def render_distributed(
        ...
        return {"rgb": colors, "depth": depths, "acc": accs}
    ...
    def _render_chunk_distributed(
        ...
        for device_id in self.device_ids:
            device_colors, device_densities = self._render_on_device(device_id, pts, rays_d, radii)
    ...
    def _render_on_device(
        ...
        for i in range(batch_size):
            for j in range(num_samples):
                ...
                selected_node = self._find_node_on_device(nodes, sample_pos, sample_radius)
...
class MemoryEfficientRenderer:
    ...
    def memory_monitor(self):
        ...
        if torch.cuda.is_available():
            ...
            if memory_used > self.available_memory_mb:
                print(
                    f"Warning: Memory usage ({memory_used:.1f} MB) exceeds limit "
    ...
    def _render_chunk_efficient(self, *args, **kwargs):
        ...
        if self.model.training:
            return torch.utils.checkpoint.checkpoint(self.model.forward, *args, **kwargs)
        else:
...
def _generate_rays_from_image(
    ...
    dirs = torch.stack(
        [(i - principal[0]) / focal[0], -(j - principal[1]) / focal[1], -torch.ones_like(i)], dim=-1
    )  # [H, W, 3]
...
class RenderingProfiler:
    ...
    def print_summary(self):
        ...
        for name, stat in stats.items():
            print(f"\n{name}:")
            print(
                f"  Time: {stat['avg_time']:.3f}±{stat['std_time']:.3f}s "
                f"(min: {stat['min_time']:.3f}s, max: {stat['max_time']:.3f}s)"
            )
            print(
                f"  Memory: {stat['avg_memory_mb']:.1f} MB average, "
