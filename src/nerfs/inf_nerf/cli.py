"""
Command Line Interface for Inf-NeRF

Provides command-line tools for training and rendering with Inf-NeRF.
"""
...
def train_cli():
    """CLI for training Inf-NeRF models."""
    parser = argparse.ArgumentParser(description="Train Inf-NeRF model")
    ...
    parser.add_argument("--use-lod", action="store_true", help="Use level of detail")
    ...
    try:
        from ..inf_nerf import (
            InfNeRFConfig,
            InfNeRF,
            InfNeRFTrainer,
            InfNeRFTrainerConfig,
        )
        from ..inf_nerf.dataset import InfNeRFDataset, InfNeRFDatasetConfig
    except ImportError as e:
        logger.error(f"Failed to import inf_nerf: {e}")
        sys.exit(1)
    ...
    try:
        ...
        train_dataset = InfNeRFDataset(dataset_config)
        val_dataset = InfNeRFDataset(InfNeRFDatasetConfig(data_root=args.data_dir, split="val"))
...
def render_cli():
    """CLI for rendering with trained Inf-NeRF models."""
    parser = argparse.ArgumentParser(description="Render with Inf-NeRF model")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "train":
            ...
            print("Usage: python -m inf_nerf.cli [train|render] [args...]")
            sys.exit(1)
    else:
        print("Usage: python -m inf_nerf.cli [train|render] [args...]")
        sys.exit(1)
