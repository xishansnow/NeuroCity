...
class MegaNeRFPlusDataset(data.Dataset):
    ...
    def __init__(
        ...
        # Scene metadata
        self.scene_bounds: torch.Tensor | None = None
        self.near_far: tuple[float, float] | None = None
    ...
    def _get_metadata(self, idx: int) -> dict[str, Any]:
        ...
        return {"index": idx, "split": self.split}
...
class PhotogrammetricDataset(MegaNeRFPlusDataset):
    ...
    def _compute_scene_bounds(self):
        ...
        self.near_far = (0.1, max_distance.item() * 2)
        ...
        if self.use_cached_rays:
            ...
            try:
                with open(cache_file, "wb") as f:
                    pickle.dump({"rays": self.rays, "rgbs": self.rgbs}, f)
            except Exception as e:
    ...
    def _generate_rays(self):
        ...
        for i, (img, pose, intrinsic) in enumerate(zip(self.images, self.poses, self.intrinsics)):
            ...
            dirs = torch.stack(
                [(i_coords - cx) / fx, -(j_coords - cy) / fy, -torch.ones_like(i_coords)], dim=-1
            )
...
class LargeSceneDataset(PhotogrammetricDataset):
    ...
    def _get_partition_images(self, partition: dict) -> list[int]:
        ...
        for i, pose in enumerate(self.poses):
            ...
            if torch.all(camera_pos >= expanded_bounds[0]) and torch.all(
                camera_pos <= expanded_bounds[1]
            ):
...
def photogrammetric_collate_fn(batch: list[dict]) -> dict[str, torch.Tensor]:
    ...
    if "rays" in batch[0]:
        ...
        return {"rays": rays, "rgbs": rgbs, "batch_size": len(batch)}
    else:
        # Image-based data
