...
def parse_args():
    ...
    parser.add_argument('--img_scale', type=float, default=0.5, help='Image scale factor')
    ...
    parser.add_argument('--ray_batch_size', type=int, default=1024, help='Number of rays per batch')
    ...
    parser.add_argument('--far', type=float, default=100.0, help='Far plane distance')
    ...
    parser.add_argument(
        '--use_appearance_matching',
        action='store_true',
        help='Use appearance matching between blocks',
    )
    parser.add_argument('--train_visibility', action='store_true', help='Train visibility network')
    ...
    # Logging and saving
    parser.add_argument('--exp_name', type=str, default='block_nerf_exp', help='Experiment name')
    parser.add_argument(
        '--save_dir',
        type=str,
        default='./experiments',
...
def setup_config(args) -> dict[str, Any]:
    ...
    # Network configuration
    network_config = {
        'hidden_dim': args.hidden_dim, 'num_layers': args.num_layers, 'pos_encoding_levels': args.pos_encoding_levels, 'dir_encoding_levels': args.dir_encoding_levels, 'appearance_dim': args.appearance_dim, 'use_integrated_encoding': True, 'skip_connections': [4]
    }
    
    # Training configuration
    training_config = {
        'learning_rate': args.learning_rate, 'weight_decay': args.weight_decay, 'scheduler': args.scheduler, 'scheduler_gamma': args.scheduler_gamma, 'grad_clip': args.grad_clip, 'visibility_weight': args.visibility_weight, 'pose_reg_weight': args.pose_reg_weight, 'appearance_reg_weight': args.appearance_reg_weight, 'use_pose_refinement': args.use_pose_refinement, 'use_appearance_matching': args.use_appearance_matching, 'train_visibility': args.train_visibility, 'interpolation_method': 'inverse_distance', 'interpolation_power': 2.0, 'render_kwargs': {
            'near': args.near, 'far': args.far, 'num_samples': args.num_samples, 'num_fine_samples': args.num_fine_samples
        }
    }
...
def main():
    ...
    train_dataset = BlockNeRFDataset(
        data_root=args.data_root, split='train', img_scale=args.img_scale, ray_batch_size=args.ray_batch_size, use_cache=True, load_appearance_ids=True, load_exposure=True
    )
    
    val_dataset = BlockNeRFDataset(
        data_root=args.data_root, split='val', img_scale=args.img_scale, ray_batch_size=args.ray_batch_size, use_cache=True, load_appearance_ids=True, load_exposure=True
    )
    ...
    block_manager = BlockManager(
        scene_bounds=scene_bounds, block_size=args.block_size, overlap_ratio=args.overlap_ratio, device=device
    )
    ...
    trainer = BlockNeRFTrainer(
        block_manager=block_manager, network_config=network_config, training_config=training_config, device=device
    )
    ...
    try:
        trainer.train(
            train_dataloader=train_loader, val_dataloader=val_loader, num_epochs=args.num_epochs, save_dir=str(
                exp_dir,
            )
        )
