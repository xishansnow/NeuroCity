...
class EnvironmentalEmbedding(nn.Module):
    ...
    def __init__(
        self,
        weather_classes: int = 10,  # sunny, cloudy, rainy, etc.
        time_of_day_dim: int = 8,
        season_classes: int = 4,  # spring, summer, fall, winter
        embedding_dim: int = 32,
    ):
        """
    ...
    def forward(
        self, weather_ids: torch.Tensor, time_of_day: torch.Tensor, season_ids: torch.Tensor
    ):
        """
...
class AdaptiveAppearanceEmbedding(nn.Module):
    ...
    def __init__(
        self, appearance_config: dict, use_exposure: bool = True, use_environmental: bool = False
    ):
        """
    ...
    def forward(
        ...
        if self.use_environmental and all(
            x is not None for x in [weather_ids, time_of_day, season_ids]
        ):
