...
class SDFDataset(data.Dataset):
    ...
    def _sample_near_surface_points(self, mesh: trimesh.Trimesh, num_points: int) -> np.ndarray:
        """在表面附近采样点"""
        try:
            # 先在表面采样
            surface_points, _ = trimesh.sample.sample_surface(mesh, num_points)
    ...
    def _compute_sdf(self, mesh: trimesh.Trimesh, points: np.ndarray) -> np.ndarray:
        """计算点的SDF值"""
        try:
...
class SyntheticSDFDataset(data.Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        # 计算SDF
        sdf = self._compute_synthetic_sdf(points, shape_type, scale, center)
...
class LatentSDFDataset(data.Dataset):
    ...
    def __init__(
        ...
        data_root: str,
        split: str = "train",
        latent_codes_path: str | None = None,
        **kwargs,
    ):
        super().__init__()
...
def create_sdf_dataloader(
    ...
    def collate_fn(batch):
        ...
        batch_dict = {"points": points, "sdf": sdf, "shape_ids": shape_ids}
