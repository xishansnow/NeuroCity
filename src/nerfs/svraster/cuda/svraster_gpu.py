...
import logging
from ..core import SVRasterConfig, SVRasterLoss
...
class SVRasterGPU:
    ...
    def _initialize_base_voxels(self):
        ...
        self.voxel_sizes.append(
            torch.full((num_voxels,), voxel_size, device=self.device, requires_grad=True)
        )
        self.voxel_densities.append(
            torch.randn(num_voxels, device=self.device, requires_grad=True) * 0.1
        )
        self.voxel_colors.append(
            torch.rand(num_voxels, 3, device=self.device, requires_grad=True) * 0.5 + 0.25
        )
    ...
    def _morton_encode_cpu(
        ...
        from ..utils.morton_utils import morton_encode_batch
    ...
    def _simple_morton_encode(self, x: torch.Tensor, y: torch.Tensor, z: torch.Tensor) -> torch.Tensor:
        """优化的Morton编码实现"""
    ...
    def _forward_cpu(
        ...
        rgb, depth = self._cpu_volume_rendering(
            ray_origins, ray_directions, voxel_positions, voxel_sizes,
            voxel_densities, voxel_colors, intersection_results
        )
    ...
    def _cpu_ray_voxel_intersection(
        self, ray_origins: torch.Tensor, ray_directions: torch.Tensor,
        voxel_positions: torch.Tensor, voxel_sizes: torch.Tensor
    ) -> Dict[str, torch.Tensor]:
    ...
    def _cpu_volume_rendering(
        self, ray_origins: torch.Tensor, ray_directions: torch.Tensor,
        voxel_positions: torch.Tensor, voxel_sizes: torch.Tensor,
        voxel_densities: torch.Tensor, voxel_colors: torch.Tensor,
    ...
    def adaptive_subdivision(self, subdivision_criteria: torch.Tensor) -> None:
        ...
        if new_voxel_count > 0:
            ...
            logger.info(f"Subdivided {new_voxel_count // 8} voxels, created {new_voxel_count} new voxels")
    ...
    def voxel_pruning(self, pruning_threshold: Optional[float] = None) -> None:
        ...
        if keep_mask.sum() < len(keep_mask):
            ...
            for level_idx in range(len(self.voxel_positions)):
                ...
                self.voxel_sizes[level_idx] = torch.full(
                    (self.voxel_positions[level_idx].shape[0],),
                    self.voxel_sizes[level_idx][level_mask].max(),
                    device=self.device,
                    requires_grad=True,
                )
    ...
    def _subdivide_level(self, level_idx: int, subdivision_mask: torch.Tensor) -> int:
        ...
        from ..utils.octree_utils import octree_subdivision
        ...
        child_densities = parent_densities.repeat_interleave(8, dim=0) * 0.8  # Inherit with some variation
        ...
        new_morton_codes = self._morton_encode_cpu(child_positions, torch.cat([self.scene_min, self.scene_max]))
        self.voxel_morton_codes[next_level] = torch.cat([self.voxel_morton_codes[next_level], new_morton_codes])
        ...
        if keep_mask.any():
            ...
            self.voxel_sizes[level_idx] = torch.nn.Parameter(
                self.voxel_sizes[level_idx][keep_mask]
            )
            self.voxel_densities[level_idx] = torch.nn.Parameter(
            ...
            self.voxel_positions[level_idx] = torch.nn.Parameter(torch.empty(0, 3, device=self.device))
            ...
            self.voxel_morton_codes[level_idx] = torch.empty(0, dtype=torch.int32, device=self.device)
        
        return int(subdivision_mask.sum().item())

    def _apply_subdivision_results(self, subdivision_flags: torch.Tensor, new_voxel_count: int) -> None:
        ...
        for level_idx, subdivision_count in enumerate(subdivision_per_level):
            if subdivision_count > 0 and level_idx < self.config.max_octree_levels - 1:
                level_flags = subdivision_flags[sum(subdivision_per_level[:level_idx]):
                                               sum(subdivision_per_level[:level_idx+1])]
    ...
    def benchmark_performance(self, num_rays: int = 1000, num_iterations: int = 10) -> Dict[str, float]:
        """
        性能基准测试
        ...
        Returns:
            性能统计结果
        """
        logger.info(f"Running performance benchmark with {num_rays} rays, {num_iterations} iterations")
...
class SVRasterGPUTrainer:
    ...
    def train_step(self, batch: Dict[str, torch.Tensor]) -> Dict[str, float]:
        ...
        with autocast(device_type="cuda" if torch.cuda.is_available() else "cpu") if torch.cuda.is_available() else nullcontext():
            outputs = self.model.forward(batch["rays_o"], batch["rays_d"])
        ...
        with torch.inference_mode():
            ...
            if "rgb" in outputs and "rgb" in batch:
