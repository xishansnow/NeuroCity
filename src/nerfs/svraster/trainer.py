...
class SVRasterTrainer:
    ...
    def __init__(
        ...
        self.loss_fn = SVRasterLoss(
            config=model.config if hasattr(model, "config") else SVRasterConfig()
        )
    ...
    def train_step(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        # 使用体积渲染器进行前向传播
        # 这里体积渲染器与训练器紧密耦合
        autocast_context = autocast(device_type="cuda") if self.config.use_amp else nullcontext()
    ...
    def train_epoch(self, dataloader: DataLoader) -> dict[str, float]:
        ...
        # 添加调试信息
        logger.info(f"Starting train_epoch with {len(dataloader)} batches")
        ...
        # 添加调试信息
        logger.info(f"Completed all batches, processing {len(epoch_losses)} loss types")

        # 计算平均损失，确保为 float
        avg_losses = {k: np.mean(v).item() for k, v in epoch_losses.items()}

        # 记录 epoch 损失（只用于日志，不影响返回类型）
        float_losses = self._convert_losses_to_float(avg_losses)
        self._log_metrics(float_losses, "train_epoch")

        self.current_epoch += 1
        logger.info(f"train_epoch completed, returning {len(avg_losses)} average losses")
        return avg_losses
    ...
    def validate(self, dataloader: DataLoader) -> dict[str, float]:
        ...
        # 计算平均损失
        avg_val_losses = {k: np.mean(v).item() for k, v in val_losses.items()}
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if (
                len(train_loader) > 0
                and epoch % max(1, self.config.save_every // len(train_loader)) == 0
            ):
        ...
        try:
            ...
            if hasattr(self.model, "voxels"):
                ...
                # 如果没有多层级数据，        检查是否有单层数据
                    ...
                    hasattr(voxels_obj, "voxel_positions") and len(voxels_obj.voxel_positions) == 1
                ):
                    # 单层级数据
                    positions = voxels_obj.voxel_positions[0]
                    sizes = voxels_obj.voxel_sizes[0]
                    densities = voxels_obj.voxel_densities[0]
                    colors = voxels_obj.voxel_colors[0]
                    morton_codes = voxels_obj.voxel_morton_codes[0]
    ...
    def _convert_losses_to_float(
        self, losses: Mapping[str, float] | Mapping[str, torch.Tensor]
    ) -> dict[str, float]:
        """
        将损失张量转换为浮点数用于日志记录
        """
