"""
Command Line Interface for SV-Raster

Provides command-line tools for training and rendering with SV-Raster.
"""
...
def train_cli():
    """CLI for training SV-Raster models."""
    parser = argparse.ArgumentParser(description="Train SV-Raster model")
    ...
    try:
        ...
        from ..svraster.dataset import SVRASTERDataset, SVRASTERDatasetConfig
    except ImportError as e:
        logger.error(f"Failed to import svraster: {e}")
        sys.exit(1)
    ...
    # Create trainer
    trainer = SVRASTERTrainer(model, trainer_config)
    ...
    try:
        ...
        train_dataset = SVRASTERDataset(dataset_config)
        val_dataset = SVRASTERDataset(SVRASTERDatasetConfig(data_root=args.data_dir, split="val"))
...
def render_cli():
    """CLI for rendering with trained SV-Raster models."""
    parser = argparse.ArgumentParser(description="Render with SV-Raster model")
    ...
    renderer = SVRASTERRenderer(model, renderer_config)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "train":
            ...
            print("Usage: python -m svraster.cli [train|render] [args...]")
            sys.exit(1)
    else:
        print("Usage: python -m svraster.cli [train|render] [args...]")
        sys.exit(1)
