...
class CNCNeRFDatasetConfig:
    ...
    def __post_init__(self):
        ...
        assert (
            len(self.pyramid_loss_weights) == self.pyramid_levels
        ), "Pyramid loss weights must match number of levels"

        assert (
            abs(self.train_split + self.val_split + self.test_split - 1.0) < 1e-6
...
class CNCNeRFDataset(Dataset):
    ...
    def _load_cameras(self) -> dict[str, Any]:
        ...
        cameras = {"intrinsics": [], "extrinsics": [], "image_names": []}
    ...
    def _generate_rays(
        ...
        rays = np.concatenate(
            [
                rays_o,  # Origin (3)
                rays_d,  # Direction (3)
                np.full((rays_o.shape[0], 1), self.config.near_far[0]),  # Near (1)
                np.full((rays_o.shape[0], 1), self.config.near_far[1]),  # Far (1)
            ],
            axis=-1,
        )
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        batch = {"rays": rays, "colors": colors, "ray_indices": ray_indices}
...
def create_synthetic_dataset(
    ...
    num_views = 100
    radius = 4.0
    cameras_data = {"camera_angle_x": 0.6911112070083618, "frames": []}
    ...
    for i in range(num_views):
        ...
        # Add to cameras data
        cameras_data["frames"].append({"file_path": img_name, "transform_matrix": c2w.tolist()})
...
def create_cnc_nerf_dataloader(
    ...
    return MultiResolutionDataLoader(dataset, batch_size=batch_size, shuffle=(split == "train",))
