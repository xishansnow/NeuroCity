...
from typing import Optional, Any, Callable, TypeVar, TypedDict, Mapping, NotRequired, cast

from occupancy_net.core import OccupancyNetwork, ConditionalOccupancyNetwork
from occupancy_net.dataset import OccupancyDataset, SyntheticOccupancyDataset
from occupancy_net.trainer import OccupancyTrainer, create_occupancy_dataloader

from nerfs.sdf_net.core import SDFNetwork, LatentSDFNetwork
from nerfs.sdf_net.dataset import SDFDataset, SyntheticSDFDataset
from nerfs.sdf_net.trainer import SDFTrainer, create_sdf_dataloader

# Type aliases for improved readability
class BatchDict(TypedDict):
    points: Tensor
    occupancy: Tensor | None
    sdf: Tensor | None

class MeshDataWithVertices(TypedDict):
    vertices: np.ndarray  # [N, 3] array of vertex positions
    faces: np.ndarray    # [M, 3] array of face indices
    occupancy_grid: NotRequired[np.ndarray]  # Optional occupancy grid
    sdf_grid: NotRequired[np.ndarray]        # Optional SDF grid

class MeshDataGridOnly(TypedDict):
    occupancy_grid: np.ndarray  # Occupancy grid when mesh extraction fails
    sdf_grid: np.ndarray       # SDF grid when mesh extraction fails

MeshDict = MeshDataWithVertices | MeshDataGridOnly

def example_occupancy_network() -> tuple[OccupancyNetwork, SyntheticOccupancyDataset]:
    """占用网络使用示例"""
    print("=" * 50)
    ...
    model = OccupancyNetwork(
        dim_input=3, dim_hidden=256, num_layers=8, use_batch_norm=True, dropout_prob=0.1
    )
    ...
    dataset = SyntheticOccupancyDataset(
        num_samples=100, num_points=10000, shape_types=['sphere', 'cube', 'cylinder']
    )
    ...
    sample_batch: BatchDict = next(iter(dataloader))
    points: Tensor = sample_batch['points']  # [B, N, 3]
    occupancy: Tensor = sample_batch['occupancy']  # type: ignore # [B, N, 1]
    ...
    # 前向传播
    with torch.no_grad():
        pred_occupancy: Tensor = model(points)
        loss: Tensor = model.compute_loss(pred_occupancy, occupancy)
    ...
    try:
        mesh_data = cast(MeshDict, model.extract_mesh(
...
def example_sdf_network() -> tuple[SDFNetwork, SyntheticSDFDataset]:
    """SDF网络使用示例"""
    print("=" * 50)
    ...
    model = SDFNetwork(
        dim_input=3, dim_latent=256, dim_hidden=512, num_layers=8, skip_connections=[4], geometric_init=True, weight_norm=True
    )
    ...
    sample_batch: BatchDict = next(iter(dataloader))
    points: Tensor = sample_batch['points']  # [B, N, 3]
    sdf: Tensor = sample_batch['sdf']  # type: ignore # [B, N, 1]
    ...
    latent_code: Tensor = torch.randn(batch_size, 256) * 0.01
    
    # 前向传播
    with torch.no_grad():
        pred_sdf: Tensor = model(points, latent_code)
        loss: Tensor = model.compute_sdf_loss(pred_sdf, sdf, loss_type='l1')
    ...
    try:
        gradient_penalty: Tensor = model.compute_gradient_penalty(
            points[:1], # 使用单个样本
            latent_code[:1], lambda_gp=0.1
        )
    ...
    try:
        mesh_data = cast(MeshDict, model.extract_mesh(
            latent_code=latent_code[:1], resolution=32, # 使用较低分辨率
            threshold=0.0
        ))
        if 'vertices' in mesh_data:
            print(f"Extracted mesh: {
...
def example_latent_sdf_network() -> LatentSDFNetwork:
    """潜在SDF网络使用示例"""
    print("=" * 50)
    ...
    latent_model = LatentSDFNetwork(
        dim_latent=256, num_shapes=100, # 预定义100个形状
        dim_hidden=512, num_layers=8
    )
    
    print(f"Latent SDF model created with {
...
def example_conditional_occupancy_network() -> ConditionalOccupancyNetwork:
    """条件占用网络使用示例"""
    print("=" * 50)
    ...
    conditional_model = ConditionalOccupancyNetwork(
        dim_input=3, dim_hidden=256, num_layers=8, dim_condition=128
    )
    
    print(f"Conditional model created with {
        sum(p.numel() for p in conditional_model.parameters()):,
    }
...
def example_training_setup() -> tuple[OccupancyTrainer, Any]:
    """训练设置示例"""
    print("=" * 50)
    ...
    occ_trainer = OccupancyTrainer(
        model=occ_model, train_dataloader=occ_dataloader, device='cpu', # 使用CPU进行演示
        log_dir='logs/demo_occupancy', checkpoint_dir='checkpoints/demo_occupancy'
    )
...
def main() -> None:
    ...
    try:
        ...
        # 输出总结信息
        print("\nSummary:")
        print(f"- Occupancy Network: {
            sum(p.numel() for p in occ_model.parameters()):,
        }
        print(f"- SDF Network: {sum(p.numel() for p in sdf_model.parameters()):, } parameters")
        print(f"- Latent SDF Network: {
            sum(p.numel() for p in latent_sdf_model.parameters()):,
        }
        print(f"- Conditional Occupancy Network: {
            sum(p.numel() for p in cond_occ_model.parameters()):,
        }
