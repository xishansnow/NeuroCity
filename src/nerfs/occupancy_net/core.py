...
@dataclass
class OccupancyNetConfig:
    """Configuration for Occupancy Net model with modern features."""
...
class OccupancyNetwork(nn.Module):
    """Network for predicting occupancy values and gradients."""
    ...
    def __init__(
        ...
        # Initialize weights for sphere occupancy
        if use_sphere_init:
            self._init_sphere_weights()

    def _init_sphere_weights(self):
        """Initialize weights to approximate a sphere occupancy."""
        with torch.no_grad():
            # Initialize first layer to compute radius
            self.layers[0].weight.data[0, :3] = 1.0  # Compute radius
            self.layers[0].bias.data[0] = 0.0

            # Initialize output layer to compute sphere occupancy
            self.occupancy_head.weight.data[0, 0] = 1.0
            self.occupancy_head.bias.data[0] = -self.sphere_radius

    def forward(self, coords: Tensor) -> TensorDict:
        """Forward pass with gradient computation."""
        # Enable gradient computation for occupancy
        ...
        gradients = torch.autograd.grad(
            occupancy, coords, grad_outputs=torch.ones_like(occupancy), create_graph=True
        )[0]

        return {"occupancy": occupancy, "gradients": gradients}
...
class OccupancyNet(nn.Module):
    """Occupancy Net model with modern optimizations."""

    def __init__(self, config: OccupancyNetConfig):
        super().__init__()
        self.config = config

        # Initialize occupancy network
    ...
    def forward(self, coords: Tensor, view_dirs: Tensor | None = None) -> TensorDict:
        ...
        for i in range(0, coords.shape[0], self.config.chunk_size):
            ...
            with autocast(enabled=self.config.use_amp):
                # Get occupancy values and gradients
                occupancy_output = self.occupancy_network(chunk_coords)
                occupancy = occupancy_output["occupancy"]
                occupancy_gradients = occupancy_output["gradients"]

                # Get colors
                if chunk_view_dirs is not None:
                    color_input = torch.cat(
                        [chunk_coords, occupancy_gradients, chunk_view_dirs], dim=-1
                    )
    ...
    def compute_loss(self, predictions: TensorDict, targets: TensorDict) -> TensorDict:
        ...
        if "occupancy" in targets:
            losses["occupancy"] = F.binary_cross_entropy_with_logits(
                predictions["occupancy"], targets["occupancy"]
            )
...
class ConditionalOccupancyNetwork(OccupancyNetwork):
    ...
    def encode_shape(self, points: torch.Tensor, occupancy: torch.Tensor) -> torch.Tensor:
        ...
        if not hasattr(self, "shape_encoder"):
            self.shape_encoder = nn.Linear(4, self.hidden_dim)
