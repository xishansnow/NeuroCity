...
def parse_args():
    ...
    parser.add_argument('--image_scale', type=float, default=0.5, help='Image scale factor')
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Scene bounds as JSON string: [x_min,
        y_min,
        z_min,
        x_max,
        y_max,
    ...
    parser.add_argument('--num_submodules', type=int, default=8, help='Number of submodules')
    parser.add_argument('--grid_size', type=str, default='4, 2', help='Grid size as "x, y"')
    parser.add_argument(
        '--overlap_factor',
        type=float,
        default=0.15,
    ...
    parser.add_argument('--far', type=float, default=1000.0, help='Far plane distance')
    
    # Logging and saving
    parser.add_argument('--exp_name', type=str, default='mega_nerf_exp', help='Experiment name')
    parser.add_argument(
        '--output_dir',
        type=str,
        default='./experiments',
    ...
    parser.add_argument('--wandb_project', type=str, default='mega-nerf', help='Wandb project name')
...
def setup_config(args) -> MegaNeRFConfig:
    ...
    config = MegaNeRFConfig(
        # Scene decomposition
        num_submodules=args.num_submodules, grid_size=(
            grid_x,
            grid_y,
        )
        hidden_dim=args.hidden_dim, num_layers=args.num_layers, skip_connections=skip_connections, use_viewdirs=args.use_viewdirs, # Training parameters
        batch_size=args.batch_size, learning_rate=args.learning_rate, lr_decay=args.lr_decay, max_iterations=args.iterations_per_submodule, # Sampling parameters
        num_coarse=args.num_coarse_samples, num_fine=args.num_fine_samples, near=args.near, far=args.far, # Appearance embedding
        use_appearance_embedding=args.use_appearance_embedding, appearance_dim=args.appearance_dim, # Scene bounds
...
def create_partitioner(args, camera_positions: np.ndarray):
    ...
    if args.partitioning_strategy == 'grid':
        ...
        partitioner = GridPartitioner(
            scene_bounds=scene_bounds, grid_size=(
                grid_x,
                grid_y,
            )
        )
        ...
        partitioner = GeometryAwarePartitioner(
            scene_bounds=scene_bounds, camera_positions=camera_positions, num_partitions=args.num_submodules, overlap_factor=args.overlap_factor, use_kmeans=args.use_kmeans
        )
    else:
...
def main():
    ...
    if args.use_wandb:
        wandb.init(
            project=args.wandb_project, name=args.exp_name, config=vars(args), dir=str(exp_dir)
        )
    
    # Load camera dataset
    print("Loading camera dataset...")
    camera_dataset = CameraDataset(
        data_root=args.data_root, split='train', image_scale=args.image_scale, load_images=True
    )
    ...
    dataset = MegaNeRFDataset(
        data_root=args.data_root, partitioner=partitioner, split='train', ray_batch_size=args.batch_size, image_scale=args.image_scale, use_cache=True, cache_dir=str(
            exp_dir / 'cache',
        )
    )
    ...
    with open(exp_dir / 'config.json', 'w') as f:
        json.dump({
            'num_submodules': config.num_submodules, 'grid_size': config.grid_size, 'overlap_factor': config.overlap_factor, 'hidden_dim': config.hidden_dim, 'num_layers': config.num_layers, 'scene_bounds': config.scene_bounds, 'batch_size': config.batch_size, 'learning_rate': config.learning_rate
        }, f, indent=2)
    ...
    if args.training_mode == 'sequential':
        trainer = MegaNeRFTrainer(
            config=config, model=model, dataset=dataset, output_dir=str(exp_dir), device=device
        )
    elif args.training_mode == 'parallel':
        trainer = ParallelTrainer(
            config=config, model=model, dataset=dataset, output_dir=str(
                exp_dir,
            )
        )
    else:
    ...
    try:
        if args.training_mode == 'sequential':
            training_stats = trainer.train_sequential(
                num_iterations_per_submodule=args.iterations_per_submodule, log_interval=args.log_interval, val_interval=args.val_interval
            )
        else:  # parallel
            training_stats = trainer.train_parallel(
                num_iterations_per_submodule=args.iterations_per_submodule, save_interval=args.save_interval
            )
