...
class UrbanRFConfig:
    ...
    # 场景参数
    scene_bounds: Tuple[float, float, float, float, float, float] = (-50, -50, -10, 50, 50, 30)  # x_min, y_min, z_min, x_max, y_max, z_max
...
class UrbanRadianceField(nn.Module):
    ...
    def _find_intersecting_dnmps(self, ray_origins: torch.Tensor, ray_directions: torch.Tensor) -> List[Tuple[int, torch.Tensor]]:
        """找到与射线相交的DNMP"""
        # 简化实现：返回所有DNMP及其采样点
        intersections = []
        ...
        for i, voxel_center in enumerate(self.voxel_grid):
            ...
            t_vals = torch.linspace(self.config.near_plane, self.config.far_plane, 
                                  self.config.n_samples, device=ray_origins.device)
            ...
            voxel_size = self.config.voxel_size
            in_voxel = torch.all(
                torch.abs(points - voxel_center.unsqueeze(0).unsqueeze(0)) <= voxel_size / 2,
                dim=-1
            )
            
            if torch.any(in_voxel):
                intersections.append((i, points[in_voxel]))
    ...
    def render_rays(self, ray_origins: torch.Tensor, ray_directions: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        # 初始化输出
        rgb_final = torch.zeros(batch_size, 3, device=device)
        depth_final = torch.zeros(batch_size, device=device)
