...
class SDFDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.coords[idx], self.sdf[idx]

def main():
    """主函数 - 演示全球特征向量库的使用"""
    logger.info("全球特征向量库演示开始...")
    ...
    # 创建全球特征库
    global_library = GlobalFeatureLibrary(config)
    ...
    # 可视化覆盖范围
    global_library.visualize_coverage("global_coverage.png")
    ...
    sdf_grid = sdf_pred.reshape((N, N, N)).cpu().numpy()
    verts, faces, normals, values = measure.marching_cubes(sdf_grid, level=0.0)
    # 可用 trimesh/pyvista 保存为 .ply/.obj
    
    logger.info("全球特征向量库演示完成")
