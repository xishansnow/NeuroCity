...
def demo_occupancy_generation():
    ...
    objects = [
        {
            'type': 'sphere',
            'params': {'center': (-15, -15, 20), 'radius': 12, 'filled': True}
        },
        {
            'type': 'cylinder',
            'params': {'center': (15, -15, 25), 'radius': 8, 'height': 30, 'filled': True}
        }
    ]
    ...
    generator.save_occupancy_grid(
        scene_occupancy, 
        "demo_outputs/demo_scene.npy",
        metadata={'description': 'Demo complex scene'}
    )
...
def demo_sdf_generation():
    ...
    geometry_configs = [
        {
            'type': 'sphere',
            'params': {'center': [0, 0, 0], 'radius': 15.0}
        },
        {
            'type': 'box',
            'params': {'center': [20, 0, 0], 'size': [10, 10, 10]}
        }
    ]
    
    train_coords, train_sdf = sdf_gen.generate_training_data(
        scene_bounds=(-30, -30, -30, 30, 30, 30),
...
def demo_surface_sampling():
    ...
    multi_res_samples = surface_sampler.sample_multi_resolution(
        coords, occupancy,
        resolution_levels=[1.0, 0.5, 0.25],
