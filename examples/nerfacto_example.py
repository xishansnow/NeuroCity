...
try:
    from src.nerfs.nerfacto import (
        NerfactoConfig, NerfactoModel, NerfactoTrainer, create_nerfacto_dataloader
    )
...
def basic_example():
    ...
    config = NerfactoConfig(
        # Model architecture
        num_layers=2, hidden_dim=64, geo_feat_dim=15, num_layers_color=3, hidden_dim_color=64, # Hash encoding
        num_levels=16, base_resolution=16, max_resolution=2048, log2_hashmap_size=19, features_per_level=2, # Proposal networks
        num_proposal_samples_per_ray=(256, 96),
        num_images=100, # Adjust based on your dataset
        appearance_embed_dim=32, use_appearance_embedding=True, # Background
        background_color="random", # Loss weights
        distortion_loss_mult=0.002, interlevel_loss_mult=1.0, orientation_loss_mult=0.0001, pred_normal_loss_mult=0.001, # Rendering
        near_plane=0.05, far_plane=1000.0, use_single_jitter=True, disable_scene_contraction=False, # Training
        max_num_iterations=30000, proposal_net_args_list=[            
            {
                "num_output_coords": 8,
                "num_levels": 5,
                "max_resolution": 128,
    ...
    with torch.no_grad():
        ...
        if 'accumulation' in outputs:
            ...
            print(f"Accumulation range: [{outputs['accumulation'].min():.4f}, {outputs['accumulation'].max():.4f}]")
...
def advanced_example():
    ...
    config = NerfactoConfig(
        ...
        num_layers=4, hidden_dim=128, geo_feat_dim=31, num_layers_color=4, hidden_dim_color=128, # Higher resolution hash encoding
        num_levels=20, base_resolution=32, max_resolution=4096, log2_hashmap_size=20, features_per_level=4, # More proposal samples
        num_proposal_samples_per_ray=(
            512,
            192,
        ),
        num_images=1000, appearance_embed_dim=64, use_appearance_embedding=True, # Different background
        background_color="white", # Adjusted loss weights
        distortion_loss_mult=0.001, interlevel_loss_mult=2.0, orientation_loss_mult=0.0005, pred_normal_loss_mult=0.005, # Extended rendering range
        near_plane=0.01, far_plane=2000.0, use_single_jitter=False, disable_scene_contraction=False, # Longer training
        max_num_iterations=100000, proposal_net_args_list=[
            {
